...
\begin{algorithm}[H]
 ...
 \While{$r < \max\_\text{iter}$}{
  ...
  \For{$k \leftarrow 1$ \KwTo $d_1$}{
   ...
   ${\q_k^{(r+1)}} \leftarrow$ Mult$\left( p_{\hat{\gamma}^{(r)}}(\bm{y}|\q^{(r+1)}) p_{\hat{\beta}^{k(r)}}(\q_{k} | \glssymbol{bx}_k) \right)$\;
   ${\q_k^{\text{MAP}(r+1)}} \leftarrow \argmax_j p_{\hat{\beta}^{k(r)}}(\q_{k}=j | \bm{x}^k)$\;
   }
  \For{$k \leftarrow d_1+1$ \KwTo $d_1+d_2$}{
   Form the contingency table T$_k^{(r)}$ of $\q_k^{(r)}$ against $\glssymbol{bbx}_k$, calculate the frequencies of each value of $({q}_{k},x_k)$\;
	${\q_k^{(r+1)}} \leftarrow$ Mult$\left( p_{\hat{\gamma}^{(r)}}(\bm{y}|\q^{(r+1)}) p_{T_k^{(r)}}(\q_{k} | \glssymbol{bx}^k) \right)$\;
