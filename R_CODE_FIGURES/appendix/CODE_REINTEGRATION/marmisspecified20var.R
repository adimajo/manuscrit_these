...
data_test_pred = array(0,c(4,m_test,21))
data_test_gen = array(0,c(4,m_test,21))
...
for (random in 1:20) {
  set.seed(random)
  y = rbinom(m,1,p0)
  data_learn_pred = array(0,c(4,m,21))
  data_learn_gen = array(0,c(4,m,21))
...
library(tikzDevice)
tikz('reintegration_simu_20var.tex', width=8, height = 3.2, engine = "pdftex")
plot(x = cut, y = tx_erreur[1,,1,1], xlim=c(0,.9), ylim=c(.02,.2), ylab = 'Error rate on test set', xlab = 'Cut-off value', pch=15, xaxt = "n", yaxt="n", type='o')
par(new=TRUE)
plot(x = cut, y = tx_erreur[1,,2,1], xlim=c(0,.9), ylim=c(.02,.2), ylab = 'Error rate on test set', xlab = 'Cut-off value', pch=17, col='green', xaxt = "n", yaxt="n", type='o')
par(new=TRUE)
plot(x = cut, y = tx_erreur[1,,3,1], xlim=c(0,.9), ylim=c(.02,.2), ylab = 'Error rate on test set', xlab = 'Cut-off value', pch=7, col=653, xaxt = "n", yaxt="n", type='o')
par(new=TRUE)
plot(x = cut, y = tx_erreur[1,,4,1], xlim=c(0,.9), ylim=c(.02,.2), ylab = 'Error rate on test set', xlab = 'Cut-off value', pch=8, col=590, xaxt = "n", yaxt="n", type='o')
par(new=TRUE)
plot(x = cut, y = tx_erreur[1,,5,1], xlim=c(0,.9), ylim=c(.02,.2), ylab = 'Error rate on test set', xlab = 'Cut-off value', pch=9, col=376, xaxt = "n", yaxt="n", type='o')

axis(1, at=pretty(cut), lab=pretty(cut), las=TRUE)
#axis(2, at=0.05, lab="0.05", las=TRUE)
axis(2, at=pretty(seq(0,0.3,0.05), n=15), lab=pretty(seq(0,0.3,0.05), n=15), las=TRUE)

legend(0,0.2, 
       pch = c(15,17,7,8,9), 
       col=c(1,"green",653,590,376),legend= c("Logistic regression","Augmentation","Reclassification","Parcelling","Gaussian mixture"),cex=0.9)
dev.off()