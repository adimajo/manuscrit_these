...
gini_test_equalfreq <- array(0,c(98,100))
...
data_tot[y=="0",2] = mvrnorm(n = sum(y=="0"), 0, 0.8)
data_tot[y=="1",2] = mvrnorm(n = sum(y=="1"), 2, 1.7)
data_tot[3] = mvrnorm(n = sum(y=="1"), 2, 1.7)
...
data_test <- data_tot[-train_ind,]
      ...
      gini_test_equalfreq[,random] <- foreach (n = 3:100, .combine='rbind', .packages = c("infotheo","Hmisc","sqldf")) %dopar% {
