...
In this Section, it is supposed the well-specified model is indeed a \gls{lr} tree, which amounts to assuming there $K^\star$ segments. The membership of an observation $\glssymbol{bx}$ to a segment $c$ is given by a tree, such that:
...
In the next Section, we propose to relax the constraint of Equation~\eqref{eq:def_c}, exactly as was done for quantizations in Chapter~\ref{chap4}, by using a continuous approximation of this discrete (and thus highly difficult to directly optimize) problem.
