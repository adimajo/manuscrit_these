...
Its original implementation is in Weka but the \textsf{R} package \rinline{RWeka} provides interfaces and wrappers to it.
...
%The `unique `side'' $1$ or $2$ of rule $r$ that yields eventually to $c$ is denoted by $\lambda(r,c)$. 
This membership is denoted by $\lambda(r,c)$ such that $\glssymbol{x}_{\sigma(r,c)} \in C_{\mathcal{P}a^r(c),\lambda(r,c)}$.
...
\begin{figure}
\resizebox{\textwidth}{!}{\input{R_CODE_FIGURES/chapitre6/titanic_tree.tex}}
...
As proposed in Chapter~\ref{chap4}, parallel to this \gls{sem} algorithm, quantizations are obtained by performing a \textit{maximum a posteriori} operation on the quantization $q_\alpha$ (see Section~\ref{subsec:relaxation}):
\[ \hat{q}_{j,h}^{c(s)} = 1 \text{ if } h = \argmax_{1 \leq h' \leq m_j} q_{\hat{\ag}_{j,h'}^{(c(s)}}, 0 \text{ otherwise,} \]
and the best logistic regression tree is chosen via the following BIC criterion, adapted from Equations~\eqref{eq:BICq} and~\eqref{eq:BICq}:
\[ s_\star = \argmin_{s\in \{1,\ldots, S\}} \sum_{c=1}^K \mbox{BIC}(\hat{\glssymbol{bth}}^{c(s)}) + K - 1.\]
...
\begin{enumerate}[(a)]
\item Two covariates $(\glssymbol{x}_1,\glssymbol{x}_2)$ are simulated from and the log odd ratio of $y$ is given by $\theta_1 \glssymbol{x}_1 + \theta_2 \glssymbol{x}_2$ where $\theta_1 = $ and $\theta_2 = $.
\item Two covariates $(\glssymbol{x}_1,\glssymbol{x}_2)$ are simulated from $\mathcal{U}(0,1)$ and a third categorical covariate $\glssymbol{x}_3$ with 6 uniformly drawn levels. For levels $1$ and $2$ of feature $\glssymbol{x}_3$, the log odd ratio of $y$ is given by $\theta_1 \glssymbol{x}_1 + \theta_2 \glssymbol{x}_2$ where $\theta_1 = $ and $\theta_2 = $. For levels $3$ and $4$, we have $\theta_1 = $ and $\theta_2 = $ and finally for levels $5$ and $6$, we set $\theta_1 = $ and $\theta_2 = $. This data generating mechanism is illustrated in Figure~\ref{fig:simu}. Results of various clustering methods developed in this Chapter are given in Table~\ref{tab:num_exp_tree}.
\end{enumerate}

In experiment (a), . In experiment (b), the proposed approach is able to recover the tree structure, as displayed on Figure~\ref{fig:tree_estim}. For both experiments, it would be useful to report confidence intervals and or bar plots as was done in Chapters~\ref{chap4} and~\ref{chap5} to derive an empirical consistency of the proposed approach.
...
\begin{table}[t]
\caption{\label{tab:num_exp_tree} Comparison of several clustering approaches w.r.t.\ the subsequent predictive performance.}
\centering
...
A subset of all applications, representative of $ $ portfolios with different scorecards, has been extracted for the purpose of the present benchmark with $n = $ observations and $d = $. The missing values have been preprocessed such that no continuous features have missing values and the categorical features have a separate and meaningful ``missing'' level. Also for simplification purposes, no quantization or interaction screening is performed so that the \gls{sem} algorithm is conducted as presented in Section~\ref{subsec:sem}.
...
On simulated data, it shows very promising results that aims at demonstrating the consistency of the approach. On real data from \gls{cacf}, .
