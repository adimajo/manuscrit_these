...
\begin{tikzpicture}[node distance=2.5cm,auto,>=latex', every node/.style={scale=0.7}, outer/.style={draw=gray,dashed,fill=green!1,thick,inner sep=45pt}]
    ...
    \path[->] (b) edge node {} (c);
    \node [int] (d) [below left=0.75cm and -0.5cm of c, align=left] {Draw ``fuzzy'' \\ candidate $\bqk ^{(s+1)}\sim p_{\gls{bth}^{(s)}}(\gls{y} | \bqk^{(s)}) p_{\ag^{(s)}}(\bqk^{(s)} | \gls{x})$};
    ...
    \node [int] (h) [below=1cm of g, align=left] {Evaluate ``hard'' \\ candidate $\mbox{BIC}(\hat{\gls{bth}}_{\q^{(s)}})$};
    \path[->] (g) edge node {} (h);
