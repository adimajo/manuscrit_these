...
\begin{figure}[!ht]
\centering \resizebox{.8\textwidth}{!}{\input{R_CODE_FIGURES/chapitre4/gini_mis.tex}}
...
%$\gls{ehmj}$ is the $h^{\text{th}}$ basis vector of $\gls{R}^{\gls{mj}}$ 
...
\paragraph{Raw categorical features} If $\gls{x}_j$ is a categorical component of $\gls{bx}$, quantization $\gls{qj}$ consists in grouping levels of $\gls{x}_j$ and the $C_{j,h}$s form a partition of the set $\gls{NO} = \{1, \dots, \gls{lj}\}$:
...
% and that there is no overlap, \textit{i.e.}\ $\nexists j,h,h'$ s.t.\ $C_{j,h} \cup C_{j,h'} = \varnothing$. 
...
\subsection{Cardinality of the quantization family} \label{subsec:cardinality_quant}
...
\end{proof}
 ...
 \end{tikzpicture}
     ...
     \caption{On the sample $\gls{bbx}$ (blue points), the two discretization functions $\q^1$ and $\q^2$ (which respective unique cutpoint $c^1_1$ and $c^2_1$ are displayed in red) take the same value and are thus equivalent w.r.t. $\mathcal{R}_{\gls{Tf}}$.}
     \label{fig:equiv}
 \end{figure}
...
In what follows, focus is given to \gls{lr} since it is a requirement from \gls{cacf} but it is applicable to any other supervised 
%(binary) 
...
%(which notation as a random variable will be made clearer in Section~\ref{sec:sem}) 
...
\paragraph{Remarks on model selection consistency} \label{par:consistency}
...
%In the case of nested models and the existence of a well-specified model in the considered family, any multiplicative constant to the number of parameters is appropriate for the asymptotic properties of BIC to hold. Indeed, suppose two nested models $M_i$, $M_t$ have parameters $|\q|_i > |\q|_t$ respectively; then, we have:
%\[ \text{BIC}_i - \text{BIC}_t \approx - \chi^2_{|\q|_i - |\q|_t} + C(|\q|_i - |\q|_t)\ln n, \]
%where $C=1$ for the BIC criterion but could be replaced by any other $C \in \gls{R}^+_\star$ since for $n \to + \infty$, $C(|\q|_i - |\q|_t)\ln n$ will dominate and reject the overly parametrized model $M_i$. These two hypotheses are not \textit{stricto sensu} true here since \textit{e.g.}\ for any global quantization order $|\q|$ there are a lot of possible univariate quantization orders $|\gls{qj}|$, but justify empirically a penalty term in the BIC criterion that ``counts'' only the logistic regression parameters in $\gls{bth}$.
...
\end{equation}
Thirdly, the component-wise nature of the quantization can be stated as a conditional independence assumption asin Figure~\ref{fig:nn}:
...
% given its cardinality calculated explicitly in Section~\ref{subsec:cardinality_quant}. 
%The well-specified model hypothesis~\eqref{hyp:true} yields for all $\gls{x}_j$, $p(\bqk_j^\star | \gls{x}_j) = 1$. Conversely, for $\bqk \in \bm{\mathfrak{Q}}_{\gls{bm}}$ such that $\bqk \cancel{\mathcal{R}_{\gls{Tf}}} \bqk^\star$, there exists a feature $j$ and an observation $\gls{xij}$ such that $p(\bqk_j | \gls{x}_j) = 0$. Consequently, the above sum, over all training observations in $\gls{Tf}$, reduces to:
...
This new formulation of the best quantization is still intractable since it requires to evaluate all quantizations in $\bm{\mathfrak{Q}}_{\gls{bm}}$ (possibly for all $\gls{bm}$) just like in Equation~\eqref{eq:BICq}.
%, although all terms except $\bqk^\star$ contribute to $0$ in the above $\argmax$. 
...
Classically, the \gls{em} can be replaced by the \acrlong{sem}~\cite{celeux1985sem} algorithm: the expectation (the sum over $\bqk \in \Q$) is approximated by the empirical distribution (up to a normalization constant) of draws $\bqk^{(1)}, \dots, \bqk^{(S)}$ from $p_{\gls{bth}}(y | \cdot) \prod_{j=1}^d p_{\ag_j}(\cdot | \gls{x}_j)$.
...
\begin{tikzpicture}[node distance=2.5cm,auto,>=latex', every node/.style={scale=0.7}, outer/.style={draw=gray,dashed,fill=green!1,thick,inner sep=45pt}]
    \node [int] (a) {Initialize $\bqk^{(0)}$};
    \node [int] (b) [right=1cm of a, align=left] {Fit LR \\ $p_{\gls{bth}}^{(s)}(\gls{y} | \bqk^{(s)})$};
    \path[->] (a) edge node {} (b);
    \node [int] (c) [right=3cm of b, align=left] {Fit continuous \\ approximation $p_{\ag_j^{(s)}}(\bqk_j^{(s)} | \gls{x}_j)$};
    \path[->] (b) edge node {} (c);
    \node [int] (d) [below left=0.75cm and -1cm of c, align=left] {Draw ``fuzzy'' \\ candidate $\bqk ^{(s+1)}\sim p_{\gls{bth}^{(s)}}(\gls{y} | \bqk^{(s)}) p_{\ag^{(s)}}(\bqk^{(s)} | \gls{x})$};
    \node (e) [below=1cm of c] {};
    \path[-] (c) edge node {} (e.center);
    \path[->] (e.center) edge node {} (d);
    \node (f) [below=1cm of b] {};
    \path[-] (d) edge node {} (f.center);
    \path[->] (f.center) edge node {} (b);
    \node [int] (g) [right=2cm of e, align=left] {Calculate ``hard'' \\ candidate $\hat{\q}^{(s)}(\cdot) = (\argmax_{h} p_{\ag_j^{(s)}}(\gls{ehmj} | \cdot))_1^d$};
    \path[->] (e.center) edge node {} (g);
    \node [int] (h) [below=1cm of g, align=left] {Evaluate ``hard'' \\ candidate $\mbox{BIC}(\hat{\gls{bth}}_{\hat{\q}^{(s)}})$};
    \path[->] (g) edge node {} (h);
    \begin{pgfonlayer}{background}
	\node[outer,fit=(b) (c) (d)] (i) {};
	\end{pgfonlayer}
    \node (j) [above right=1cm of b, align=left] {As long as the budget $S$ is not over};
\end{tikzpicture}
\caption{\label{fig:schema_sem} Schema of the SEM quantization approach.}
\end{figure}
