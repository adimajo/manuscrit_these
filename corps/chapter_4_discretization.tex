...
Each of the univariate quantizations $\gls{qj}$'s is a vector of $\gls{mj}$ dummies: 
...
\end{align}
where $\gls{mj}$ is an integer, denoting the number of intervals / groups to which $\gls{x}_j$ is mapped, $\gls{ehmj}$ is the $h^{\text{th}}$ basis vector of $\gls{R}^{\gls{mj}}$ and the sets $C_{j,h}$ are defined with respect to each feature type as is described just below.
...
\begin{tikzpicture}[node distance=2.5cm,auto,>=latex', every node/.style={scale=0.7}, outer/.style={draw=gray,dashed,fill=green!1,thick,inner sep=45pt}]
    \node [int] (a) {Initialize $\bqk$};
    \node [int] (b) [right=1cm of a, align=left] {Fit LR \\ $p_{\gls{bth}}(\gls{y} | \bqk)$};
    \path[->] (a) edge node {} (b);
    \node [int] (c) [right=3cm of b, align=left] {Fit continuous \\ approximation $p_{\ag_j}(\bqk_j | \gls{x}_j)$};
    \path[->] (b) edge node {} (c);
    \node [int] (d) [below left=0.75cm and -0.5cm of c, align=left] {Draw ``fuzzy'' \\ candidate $\bqk \sim p_{\gls{bth}}(\gls{y} | \bqk) p_{\ag}(\bqk | \gls{x})$};
    ...
    \node [int] (h) [below=1cm of g, align=left] {Evaluate ``hard'' \\ candidate $\mbox{BIC}(\hat{\gls{bth}}_{\q})$};
    \path[->] (g) edge node {} (h);
    \begin{pgfonlayer}{background}
	\node[outer,fit=(b) (c) (d)] (i) {};
	\end{pgfonlayer}
    \node (j) [above right=1cm of b, align=left] {As long as the budget $T$ is not over};
\end{tikzpicture}
