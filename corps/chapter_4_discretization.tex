...
\end{tikzpicture}
\caption{\label{fig:dep}Dependence structure between $\glssymbol{X}_j$,$Q_j$ and $\glssymbol{Y}$} 
\end{minipage}
...
Let $\q^1$ and $\q^2$ in $\Q$ such that $\q^1 \mathcal{R}_{\glssymbol{T}_n} \q^2 \equiv \forall i,j \; \q^1_j(\glssymbol{xij}) = \q^2_j(\glssymbol{xij})$. See Figure~\ref{fig:equiv} for an example.

\subparagraph{Lemma} Relation $\mathcal{R}_{\glssymbol{T}_n}$ defines an equivalence relation on $\Q$.

\begin{proof}
Relation $\mathcal{R}_{\glssymbol{T}_n}$ is trivially reflexive and symmetric because of the reflexive and symmetric nature of the equality relation in $\glssymbol{R}$: $\forall i,j \; \q^1_j(\glssymbol{xij}) = \q^1_j(\glssymbol{xij})$ and $\forall i,j \; \q^1(\glssymbol{xij}) = \q^2(\glssymbol{xij})$. Similarly, let $\q^3 \in \Q$ such that $\q^1 \mathcal{R}_{\glssymbol{T}_n} \q^3  \equiv \forall i,j \; \q^1_j(\glssymbol{xij}) = \q^3_j(\glssymbol{xij})$. Again, we immediately get $\forall i,j \; \q^2_j(\glssymbol{xij}) = \q^3_j(\glssymbol{xij})$, \textit{i.e.}\ $\q^2 \mathcal{R}_{\glssymbol{T}_n} \q^3$ which proves the transitivity of $\mathcal{R}_{\glssymbol{T}_n}$.
\end{proof}
 ...
 \end{tikzpicture}
     ...
     \caption{On the sample $\glssymbol{bbx}$ (blue points), the two discretization functions $\q^1$ and $\q^2$ (which respective unique cutpoint $c^1_1$ and $c^2_1$ are displayed in red) take the same value and are thus equivalent w.r.t. $\mathcal{R}_{\glssymbol{T}_n}$.}
     \label{fig:equiv}
 \end{figure}
...
\end{align*}
The well-specified model hypothesis~\eqref{hyp:true} yields for all $\glssymbol{x}_j$, $p(\bqk_j^\star | \glssymbol{x}_j) = 1$. Conversely, for $\bqk \in \Q$ such that $\bqk \cancel{\mathcal{R}_{\glssymbol{T}_n}} \bqk^\star$, there exists a feature $j$ and an observation $\glssymbol{xij}$ such that $p(\bqk_j | \glssymbol{x}_j) = 0$. Consequently, the above sum, over all training observations in $\glssymbol{T}_n$, reduces to:
\begin{align*}
