...
This new formulation of the best quantization is still untractable since it requires to evaluate all quantizations in $\Q$, although all terms except $\bqk^\star$ contribute to $0$ in the above $\argmax$. In the misspecfied model-case however, there is no such garantee but it can still be claimed that the best candidate $\q^\star$ in terms of criterion~\eqref{eq:BICq} dominates the sum.

Our goal in the next Section is to generate $\bbqk$ from $p(\glssymbol{bby} | \cdot) \prod_{j=1}^d p(\cdot | \glssymbol{bbx}_j)$. Among other things detailed later on, we shall . If the resulting MCMC is efficient, $\q_\star$ will be the mode of the empirical distribution of generated candidates, which, as in Section~\ref{sec:proposal} with the neural network approach, can be selected with the BIC criterion~\eqref{eq:BICq}. Using \eqref{hyp:true}, it seems most natural to use a logistic regression for $p( y | \bqk)$. Following Section~\ref{sec:proposal} and as was empirically argumented in Section~\ref{subsec:relaxation}, $p(\bqk | x_j)$ will be parametrized as a poytomous logistic regression, similar to the softmax function of Equation~\eqref{eq:softmax} without the over-parametrization (one level per feature $j$, say $m_j$, is considered reference). Similarly, $p(\bqk | x_j)$ are no more step functions but smooth functions as in Figure~\ref{fig:MAP}.
...
However, as was the case in Section~\ref{sec:proposal}, we are not truly interested in maximizing this likelihood: 
