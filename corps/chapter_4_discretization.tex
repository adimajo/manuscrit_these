...
where $C=1$ for the BIC criterion but could be replaced by any other $C \in \mathbb{R}^+_\star$ since for $n \to + \infty$, $C(|\q|_i - |\q|_t)\ln n$ will dominate and reject the overly parametrized model $M_i$.

For all these reasons, we use in the remainder of this manuscript the BIC criterion which penalizes only on the logistic regression parameters $\glssymbol{bth}$.
