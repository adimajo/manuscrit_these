...
p(\glssymbol{bbx}, \glssymbol{bby} | \bdelta) & = \int_{\Theta} p_{\glssymbol{bth}}(\glssymbol{bby} | \glssymbol{bbx}, \bdelta) p(\glssymbol{bth} | \bdelta)d\glssymbol{bth} + p(\glssymbol{bx}) \\
& = \exp(-\text{BIC}(\hat{\glssymbol{bth}}_{\bdelta})/2 + O(1)) + p(\glssymbol{bx})
\end{align*}
...
%}
Let $\bdelta^{(s+1)} \leftarrow \begin{cases} \bdelta' \text{ with probability } \min(1,\alpha), \\ \bdelta^{(s)} \text{ with probability } \max(0,1-\alpha).  \end{cases}$\;
}
