...
\end{enumerate}
Note that the interaction screening procedure is also applicable to continuous features, which is not tested here. These 2 experiments are run 100 times with $n = \{1{,}000,10{,}000\}$ and histograms are given in Table~\ref{tab:simu_inter}.
...
Half of the datasets do not benefit from the enriched model space but the performance of \textit{glmdisc} without interactions was already high. The other half 
