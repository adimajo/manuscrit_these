...
%En accord avec ce qui précède, on se limite au cas $\gls{Xspace} \subseteq\gls{R}^d$.
...
\begin{tikzpicture}
    \begin{axis}[
        ...
        xlabel=x,
        ylabel={$p(\gls{Y}=1 | \gls{bX} = \gls{bx})$},
        tick align=outside,
        enlargelimits=false]
...
%Si l'on note $\mathcal{B}(\gls{Xspace} \times \mathcal{Y})$ la tribu borélienne sur $\gls{Xspace} \times \mathcal{Y}$, alors $p$, dont $\pd$ est un sous-produit, est l'une des nombreuses mesures de probabilités possibles sur l'espace mesurable $(\gls{Xspace} \times \mathcal{Y},\mathcal{B}(\gls{Xspace} \times \mathcal{Y}))$. 
...
\begin{tikzpicture}[scale=1.1,every node/.style={minimum size=1cm},on grid]
	...
	\begin{scope}[
		...
		\fill[black]
			...
			(2.1,1.9) node[below,scale=2]{$\gls{bthstar}$};
...
N'ayant accès à $\pd(\cdot | \gls{bx})$ qu'à travers un échantillon, il nous faut développer un critère empirique à partir du critère théorique (on parle aussi de critère asymptotique) donné ici.

\subsection{Minimisation du risque empirique et maximum de vraisemblance} \label{subsec:gradient}

On peut réécrire $\gls{KL}(p_{\text{data}}(\cdot|\gls{bx})||p_{\gls{bth}}(\cdot|\gls{bx}))$ pour faire apparaître une quantité indépendante de $p_{\gls{bth}}$ :
%\[ \gls{KL}(p_{\text{data}}(\cdot|\gls{bx})||p_{\gls{bth}}(\cdot|\gls{bx})) = \underbrace{\sum_{y \in \{0,1\}} p_{\text{data}}(y|\gls{bx}) \ln [p_{\text{data}}(y|\gls{bx})]}_{\perp p_{\gls{bth}}} - \underbrace{\sum_{y \in \{0,1\}} p_{\text{data}}(y|\gls{bx}) \ln [p_{\gls{bth}}(y|\gls{bx})]}_{\mathbb{E}_{\gls{Y}} [\ln[p_{\gls{bth}}(\cdot|\gls{bx})]]}  \]
...
%On note $p_{\mathcal{X}}$ la \gls{pdf} marginale de $\gls{X}$ et on définit pour tout $f: \mathcal{X} \to \mathbb{R}$ la norme $||f||_2^2 = \int_{\mathcal{X}} f dp_{\mathcal{X}}$. Par Pythagore, on obtient facilement pour tout $y$ :
%\begin{equation} \label{eq:biais_variance}
%||p_{\hat{\gls{bth}}}(y|\cdot) - \pd(y|\cdot)||_2^2 = \underbrace{||p_{\hat{\gls{bth}}}(y|\cdot) - p_{\gls{bth}^\star}(y|\cdot)||_2^2}_{\text{erreur d'estimation}} + \underbrace{||p_{\gls{bth}^\star}(y|\cdot) - \pd(1|\cdot)||_2^2}_{\text{biais de modèle}}.
%\end{equation}
\begin{align}
 & \mathbb{E}_{\gls{T}_n} \mathbb{E}_{\gls{bx}} [p_{\hat{\gls{bth}}}(y| \gls{bx}) - \pd(y| \gls{bx})]^2  \nonumber \\
= & \mathbb{E}_{\gls{bx}} [ \underbrace{[\pd(y| \gls{bx}) - \mathbb{E}_{\gls{T}_n} [ p_{\hat{\gls{bth}}}(y| \gls{bx})]]^2}_{\text{biais de modèle}} + \underbrace{\mathbb{E}_{\gls{T}_n} [[ p_{\hat{\gls{bth}}}(y| \gls{bx}) - \mathbb{E}_{\gls{T}_n} [ p_{\hat{\gls{bth}}}(y| \gls{bx}) ]]^2]}_{\text{erreur d'estimation}} ] \label{eq:bias1} \\
= & \mathbb{E}_{\gls{bx}} [ \underbrace{[\pd(y| \gls{bx}) - p_{\gls{bthstar}}(y| \gls{bx})]^2}_{\text{biais de modèle}} + \underbrace{\mathbb{E}_{\gls{T}_n} [[ p_{\hat{\gls{bth}}}(y| \gls{bx}) - p_{\gls{bthstar}}(y| \gls{bx})  ]^2}_{\text{variance}} ]]. \label{eq:bias2}
\end{align}
Pour la dérivation rigoureuse de ce résultat, se référer à~\cite{schutze2008introduction} (p.\ 308–314). Le passage de~\ref{eq:bias1} à~\ref{eq:bias2} est garanti par le caractère asymptotiquement sans biais de l'estimateur du maximum de vraisemblance, même dans le cas du modèle mal spécifié~\cite{white1982maximum}. Autrement dit, pour $n$ assez grand, on a $\sqrt{n} (\hat{\gls{bth}} - \gls{bthstar}) \sim \mathcal{N}(\bm{0} , \Sigma)$ ce qui garantit $|| \hat{\gls{bth}} - \gls{bthstar}||_2 \to 0$ et $\mathbb{E}_{\gls{T}_n} [ p_{\hat{\gls{bth}}}(y| \gls{bx})] = p_{\gls{bthstar}}(y| \gls{bx})$.
...
\begin{tikzpicture}[scale=1.1,every node/.style={minimum size=1cm},on grid]
	...
	\begin{scope}[
		...
		\fill[black]
			...
			(2.1,1.9) node[below,scale=2]{$\gls{bthstar}$}
			(2.2,4.3) node [scale=2] {$\hat{\gls{bth}}$} ;
		\fill[blue]
...
%On peut faire apparaître ce phénomène en mesurant l'écart quadratique moyen entre le meilleur modèle $p_{\gls{bth}^\star}$ et le modèle proposé $p_{\hat{\gls{bth}}}$; en reprenant les notations de la section précédente, on a~:
%\begin{align*}
%||p_{\gls{bth}^\star}(y|x) - p_{\hat{\gls{bth}}}(y|x)||_2^2 & = \text{Var}(p_{\text{data}}(y|x)) & + (p_{\text{data}}(y|x) - \mathbb{E}[p_{\gls{bth}}(y|x)])^2 & + \text{Var}(p_{\gls{bth}}(y|x)) \\
...
%
%D'après la description faite dans le paragraphe précédent, les \gls{svm} ne sont pas si différents de la \gls{lr} en ce qu'ils cherchent une frontière linéaire entre les deux classes. Pour des raisons théoriques exposées en~\lnameref{ccl}, à $n$ fixé, la ``séparabilité'' des données, autrement dit la probabilité d'existence de ladite frontière linéaire, augmente avec la dimension $d$ de l'espace des covariables.
%
%Il est donc tentant d'augmenter cette dimension par construction, à partir des covariables d'origine, en créant diverses combinaisons. Une approche possible est l'utilisation de noyaux, intimement liés aux \gls{svm} : on applique une transformation non-linéaire $\q$ au vecteur d'entrée $\gls{bx}$ dont l'espace d'arrivée est ``grand''. En réexprimant l'équation~\ref{eq:svm} à l'aide de multiplicateurs de Lagrange (voir les développements techniques dans~\cite{scholkopf2002learning}), on obtient la formulation dite \textit{duale} des \gls{svm} qui ne fait intervenir que des produits scalaires entre les observations $1 \leq i,i' \leq n$, $\q(\gls{bx}_i)' \q(\gls{bx}_i')$. C'est ce dernier produit scalaire que l'on écrit sous la forme d'un noyau $K(\gls{bx}_i,\gls{bx}_i')$, dont le calcul se fait dans l'espace d'origine et qui ne nécessite pas la connaissance explicite de $\q$.
%
%Dans les expériences du chapitre~\ref{chap2}, on utilise le noyau gaussien $K(\gls{bx}_i,\gls{bx}_i') = \exp \left( - \frac{|| \gls{bx}_i - \gls{bx}_i' ||_2^2}{2 \sigma^2} \right)$.
