...
N'ayant accès à $\pd(\cdot | \gls{bx})$ qu'à travers un échantillon, il nous faut développer un critère empirique à partir du critère théorique (souvent de nature asymptotique) donné ici.

\subsection{Minimisation du risque empirique et maximum de vraisemblance} \label{subsec:gradient}

On peut réécrire $\gls{KL}(\pd(\cdot|\gls{bx})||p_{\gls{bth}}(\cdot|\gls{bx}))$ pour faire apparaître une quantité indépendante de $p_{\gls{bth}}$ :
%\[ \gls{KL}(\pd(\cdot|\gls{bx})||p_{\gls{bth}}(\cdot|\gls{bx})) = \underbrace{\sum_{y \in \{0,1\}} \pd(y|\gls{bx}) \ln [\pd(y|\gls{bx})]}_{\perp p_{\gls{bth}}} - \underbrace{\sum_{y \in \{0,1\}} \pd(y|\gls{bx}) \ln [p_{\gls{bth}}(y|\gls{bx})]}_{\mathbb{E}_{\gls{Y}} [\ln[p_{\gls{bth}}(\cdot|\gls{bx})]]}  \]
...
%& \geq \ell(\tilde{\gls{bth}};\gls{bbx},\gls{bby}). \qedhere
%\end{align*}
%\end{proof}
...
On note la matrice hessienne de $\ell$ en $\gls{bth}$ par $\mathbf{H}_{\gls{bth}} = \left( \dfrac{\partial^2 \ell}{\partial \theta_j \partial \theta_k} \right)_{0 \leq j,k \leq d}$. Le développement de Taylor, qui revient à considérer que la log-vraisemblance est localement quadratique, donne à l'étape $(s)$ :
...
%\begin{align*}
%||p_{\gls{bth}^\star}(y|x) - p_{\hat{\gls{bth}}}(y|x)||_2^2 & = \text{Var}(\pd(y|x)) & + (\pd(y|x) - \mathbb{E}[p_{\gls{bth}}(y|x)])^2 & + \text{Var}(p_{\gls{bth}}(y|x)) \\
